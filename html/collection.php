<?php include_once 'head.php'; ?>
      ...
      <section id="main-content">
          <section class="wrapper">
              ...
              <div class="row state-overview">
                  ...
                  <?php } ?>
                  <form action="index.php?m=collection&a=getCollect" method="post" onsubmit="return file.check();">
                      		...
                      		<li id="li_<?php echo $v['id']; ?>">
                          		<div class="listTableIn pull-left">
                                    <div class="listTableInL pull-left">
                                        ...
                                        <div class="name">
                                            <a target="_blank" href="index.php?a=own&urlkey=<?php echo base_convert($v['mapId'], 10, 36); ?>"><em class="<?php echo $v['icon']; ?>"></em></a>
                                            <span class="div_pro">
                                                <a target="_blank" href="index.php?a=own&urlkey=<?php echo base_convert($v['mapId'], 10, 36); ?>"><?php echo htmlspecialchars($v['name'], ENT_NOQUOTES); ?></a>
                                            </span>
                                        </div>
                                    </div>
                                    <div class="listTableInR pull-right">
                                        <div class="size"><?php echo $v['size']; ?></div>
                                        <div class="updateTime"><?php echo $v['collectTime']; ?></div>
                                    </div>
                                </div>
                            </li>
                            <?php }
    ...
    <?php if (!$urlkey) { ?>
    $("#size, #collectTime").click(function() {
        var by;
        ...
        $.ajax({
            url : 'index.php?m=collection&a=getCollect',
            type : 'POST',
            data : {
                order : $(this).attr('id'),
                by : by,
    ...
    function page(type) {
        ...
        if (type == -1) {
            href = 'index.php?m=collection&a=getCollect&urlkey=<?php echo htmlspecialchars($_REQUEST['urlkey'], ENT_NOQUOTES); ?>&search=<?php echo htmlspecialchars($search, ENT_NOQUOTES); ?>&curPage=<?php echo max($curPage-1, 1); ?>&order=' + order + '&by=' + by;
        } else if (type == 0) {
            href = 'index.php?m=collection&a=getCollect&urlkey=<?php echo htmlspecialchars($_REQUEST['urlkey'], ENT_NOQUOTES); ?>&search=<?php echo htmlspecialchars($search, ENT_NOQUOTES); ?>&curPage=<?php echo min(max($curPage, 1)+1, $page); ?>&order=' + order + '&by=' + by;
        }
