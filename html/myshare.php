<?php include_once 'head.php'; ?>
      ...
      <section id="main-content">
          <section class="wrapper">
              ...
              <div class="row state-overview">
                  ...
                  <?php } ?>
                  <form action="index.php?m=share&a=getMyShare" method="post" onsubmit="return file.check();">
                            ...
                            ?>
                      		<li id="li_<?php echo $v['id']; ?>">
                          		<div class="listTableIn pull-left" onmouseenter="$('#box_<?php echo $v['mapId']; ?>').show();" onmouseleave="$('#box_<?php echo $v['mapId']; ?>').hide();" <?php if (!$urlkey) { ?>onclick="show(<?php echo $v['id']; ?>);" <?php } ?>>
                                      ...
                                      <div class="name">
                                          <a target="_self" <?php if ($v['type'] == 2) { ?>data-lightbox="img_<?php echo $v['mapId']; ?>"<?php } ?> <?php if ($v['isdir']) { ?> href="index.php?m=share&a=getMyShare&urlkey=<?php echo base_convert($v['mapId'], 10, 36); ?>&shareTime=<?php echo $v['shareTime']; ?>" <?php } elseif ($v['type'] == 2) { ?> href="index.php?a=view&urlkey=<?php echo base_convert($v['mapId'], 10, 36); ?>" <?php } else { ?> href="index.php?a=down&urlkey=<?php echo base_convert($v['mapId'], 10, 36); ?>" <?php } ?> id="a_<?php echo $v['id']; ?>"><em class="<?php echo $v['icon']; ?>"></em></a>
                                          <span class="div_pro">
                                              <a target="_self" <?php if ($v['type'] == 2) { ?>data-lightbox="roadtrip"<?php } ?> <?php if ($v['isdir']) { ?> href="index.php?m=share&a=getMyShare&urlkey=<?php echo base_convert($v['mapId'], 10, 36); ?>&shareTime=<?php echo $v['shareTime']; ?>" <?php } elseif ($v['type'] == 2) { ?> href="index.php?a=view&urlkey=<?php echo base_convert($v['mapId'], 10, 36); ?>" <?php } else { ?> href="index.php?a=down&urlkey=<?php echo base_convert($v['mapId'], 10, 36); ?>" <?php } ?> id="a_<?php echo $v['id']; ?>"><?php echo htmlspecialchars($v['name'], ENT_NOQUOTES); ?></a>
                                          </span>
                                      </div>
                                  </div>
    ...
    $("#view, #down, #shareTime, #saveNum").click(function(){
        ...
        $.ajax({
            url : 'index.php?m=share&a=getMyShare',
            type : 'POST',
            data : {
                order : $(this).attr('id'),
                by : by,
    ...
    function page(type) {
        ...
        if (type == -1) {
            href = 'index.php?m=share&a=getMyShare&urlkey=<?php echo htmlspecialchars($urlkey, ENT_NOQUOTES); ?>&search=<?php echo htmlspecialchars($search, ENT_NOQUOTES); ?>&curPage=<?php echo max($curPage-1, 1); ?>&order=' + order + '&by=' + by;
        } else if (type == 0) {
            href = 'index.php?m=share&a=getMyShare&urlkey=<?php echo htmlspecialchars($urlkey, ENT_NOQUOTES); ?>&search=<?php echo htmlspecialchars($search, ENT_NOQUOTES); ?>&curPage=<?php echo min(max($curPage, 1)+1, $page); ?>&order=' + order + '&by=' + by;
        }
