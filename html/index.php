                            ...
                            ?>
                      		<li id="li_<?php echo $v['id']; ?>">
                          		<div class="listTableIn pull-left" onmouseenter="$('#box_<?php echo $v['id']; ?>').show();" onmouseleave="$('#box_<?php echo $v['id']; ?>').hide();">
                                      ...
                                      <div class="name">
                                          <a target="_self" id="a_<?php echo $v['id']; ?>" <?php if ($v['type'] == 2) { ?>data-lightbox="img_<?php echo $v['id']; ?>"<?php } ?> <?php if ($v['isdir']) { ?> href="index.php?path=<?php echo (trim($_REQUEST['path'], '/') ? trim($_REQUEST['path'], '/') . '/' : '') . htmlspecialchars($v['name'], ENT_NOQUOTES); ?>" <?php } elseif ($v['type'] == 2) { ?> href="index.php?a=view&urlkey=<?php echo base_convert($v['id'], 10, 36); ?>" <?php } else { ?> href="index.php?a=down&urlkey=<?php echo base_convert($v['id'], 10, 36); ?>" <?php } ?> id="p_<?php echo $v['id']; ?>"><?php if ($v['share']) { ?><div class="shareFd"></div><?php } ?><em class="<?php echo $v['icon']; ?>"></em></a>
                                      		<span class="div_pro">
                                                <a id="sa_<?php echo $v['id']; ?>" target="_self" <?php if ($v['type'] == 2) { ?>data-lightbox="img_<?php echo $v['id']; ?>"<?php } ?> <?php if ($v['isdir']) { ?> href="index.php?path=<?php echo (trim($_REQUEST['path'], '/') ? trim($_REQUEST['path'], '/') . '/' : '') . htmlspecialchars($v['name'], ENT_NOQUOTES); ?>" <?php } elseif ($v['type'] == 2) { ?> href="index.php?a=view&urlkey=<?php echo base_convert($v['id'], 10, 36); ?>" <?php } else { ?> href="index.php?a=down&urlkey=<?php echo base_convert($v['id'], 10, 36); ?>" <?php } ?>><?php echo htmlspecialchars($v['name'], ENT_NOQUOTES); ?></a>
                                  		    </span>
                                  		</div>
                                  </div>
