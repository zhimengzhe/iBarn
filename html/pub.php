<?php include_once 'head.php'; ?>
                            ...
                            ?>
                      		<li id="li_<?php echo $v['id']; ?>">
                          		<div class="listTableIn pull-left" onmouseenter="$('#box_<?php echo $v['mapId']; ?>').show();" onmouseleave="$('#box_<?php echo $v['mapId']; ?>').hide();">
                                      ...
                                      <div class="name">
                                          <a id="a_<?php echo $v['id']; ?>" target="_blank" href="index.php?a=own&urlkey=<?php echo base_convert($v['mapId'], 10, 36); ?>"><?php if ($v['collect']) { ?><i class="icon-star starFd"></i><?php } ?><em class="<?php echo $v['icon']; ?>"></em></a>
                                          <span class="div_pro">
                                    ...
                                    <div class="listTableInR pull-right">
                                      <div class="share"><?php echo number_format($v['view']); ?></div>
                                      <div class="size"><?php echo number_format($v['down']); ?></div>
                                      <div class="size"><?php echo number_format($v['saveNum']); ?></div>
                                      <div class="updateTime"><?php echo $v['shareTime']; ?></div>
                                      <div style="display:none;position: absolute;margin-left: -40px;" class="float_box" id="box_<?php echo $v['mapId']; ?>">
                                          <ul class="control">
                                              <li><a href="#" alt="<?php echo t('收藏'); ?>" data-target="#myModal1" data-toggle="modal" onclick="$('#sid').val(<?php echo $v['id']; ?>);"><i class="icon-star"></i></a></li>
                                          </ul>
                                      </div>
                                    </div>
                                </div>
    ...
    function page(type) {
        ...
        if (type == -1) {
            href = 'index.php?m=share&a=getPub&urlkey=<?php echo $urlkey; ?>&search=<?php echo $search; ?>&curPage=<?php echo max($curPage-1, 1); ?>&order=' + order + '&by=' + by;
        } else if (type == 0) {
            href = 'index.php?m=share&a=getPub&urlkey=<?php echo $urlkey; ?>&search=<?php echo $search; ?>&curPage=<?php echo min(max($curPage, 1)+1, $page); ?>&order=' + order + '&by=' + by;
        }
